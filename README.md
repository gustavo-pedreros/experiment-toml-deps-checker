...
- **Module usage map** _(opt-in: `--module-usage`)_ — static analysis of `build.gradle(.kts)` files; shows how many modules use each library via `implementation`, `api`, or test configurations; identifies the heaviest dependency modules. Recognises both dotted (`libs.androidx.core.ktx`) and camelCase (`libs.androidxCoreKtx`) accessor forms, plus bundle expansions (`libs.bundles.compose` credits every member library); unreadable build files surface as a `MOD-001` finding instead of poisoning the scan
